...
| 178 | **A realm with no army in the field keeps nobody back.** Owner-specified: every faction musters its starting unit and goes and takes the ground around it, whatever its personality. The old rule tried to say this and stopped one short — it relaxed a Defensive realm's garrison down to *one*, which is exactly what every realm starts with, so `spare` was zero in January 1350 and **no realm on the map ever mustered its opening unit.** It sat behind a wall until a second was recruited, which at 0.19.0's income is a decade. Measured after: all twelve rivals field an army by month 3 and claimed ground goes from 58 tiles to 112 in three years. Settlements are not left open — their derived defenders are free and immobile and were always the real garrison | 31 |
| 179 | **A unit's wage may take at most a quarter of a realm's net income.** Replaces "the realm must be able to afford two of them", which measured the margin against *the unit*, so the bar rose with the price of what was being bought and the expensive half of the roster stayed permanently out of reach. Measured: the median realm cleared 8 gold a month at 1370 and 21 at 1410 against a bar of 20 for Light Infantry and **100 for Heavy Cavalry** — which is the whole of the monoculture NEXT.md had been carrying since 0.19.0, and it was never the roll of three (130). The margin is now a share of the realm, which is scale-free and leaves a quarter of the income standing so recruiting never stops a realm growing (164). **Tighter, not looser** — the counter-intuitive half: loosening it to three quarters made the monoculture *worse*, because realms simply flood the cheapest unit and never accumulate the income that would buy a better one | 31 |
